...
export async function POST(req: Request) {
  ...
  try {
    ...
    const generationPromise = (async () => {
      ...
      const t0 = Date.now();
      // ⚡ Bolt Optimization: Wrap getCloudflareEmbedding in timeoutPromise to eagerly cancel
      // the network request on timeout, preventing "zombie" connections in the Edge runtime.
      const queryVector = await timeoutPromise(
        (signal) => getCloudflareEmbedding(text, cfAccountId, cfApiToken, signal),
        5000,
        "Cloudflare Embedding Timeout (5s)"
      );
