...
export async function POST(req: Request) {
  ...
  try {
    ...
    const generationPromise = (async () => {
      ...
      const t0 = Date.now();
      // ⚡ Bolt Optimization: Wrap network call in timeoutPromise to eagerly cancel the
      // underlying zombie fetch request on timeout, preventing memory leaks and blocking
      // the request coalescing cache.
