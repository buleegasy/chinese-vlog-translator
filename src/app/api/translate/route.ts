...
export async function POST(req: Request) {
  ...
  try {
    ...
    const generationPromise = (async () => {
      ...
      // ⚡ Bolt Optimization: Eagerly cancel zombie network requests on timeout.
      // Eagerly passing an AbortSignal to the underlying fetch prevents zombie requests
      // from consuming background CPU/memory, and critically prevents the Request Coalescing
      // Map (pendingTranslations) from being permanently blocked if the fetch hangs.
      const queryVector = await timeoutPromise(
        (signal) => getCloudflareEmbedding(text, cfAccountId, cfApiToken, signal),
        8000,
        "Cloudflare Embedding Timeout (8s)"
      );
