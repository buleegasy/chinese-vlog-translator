...
// 超时控制包装器
// ⚡ Bolt Optimization: Aggressive Network Request Cancellation
// The timeout wrapper now uses an AbortController. Instead of just returning a timed-out promise
// while the underlying network request continues as a "zombie" (consuming edge worker memory, CPU,
// and rate limits), the timeout now actively aborts the fetch/SDK calls to immediately free up resources.
