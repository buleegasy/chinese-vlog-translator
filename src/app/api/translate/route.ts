...
export async function POST(req: Request) {
  ...
  try {
    ...
    const generationPromise = (async () => {
      ...
      const t0 = Date.now();
      // ⚡ Bolt Optimization: Wrap CF embedding call in timeoutPromise and pass the AbortSignal
      // to eagerly cancel underlying network requests on timeout (10s). This prevents
      // "zombie" connections from silently eating memory and blocking the coalescing cache.
      const queryVector = await timeoutPromise(
        (signal) => getCloudflareEmbedding(text, cfAccountId, cfApiToken, signal),
        10000,
        "Cloudflare API 响应超时 (10s)"
      );
