...
export async function POST(req: Request) {
  ...
  try {
    ...
    const generationPromise = (async () => {
      ...
      const t0 = Date.now();
      // ⚡ Bolt Optimization: Eager Network Cancellation
      // Use timeoutPromise wrapper to pass an AbortSignal to fetch. This guarantees that if the
      // Cloudflare API hangs, we actively close the connection instead of just leaking a pending
      // request and permanently blocking the request coalescing Map for this text.
      const queryVector = await timeoutPromise(
        (signal) => getCloudflareEmbedding(text, cfAccountId, cfApiToken, signal),
        8000,
        "Cloudflare Embedding 响应超时 (8s)"
      );
