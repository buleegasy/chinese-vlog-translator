...
export async function POST(req: Request) {
  ...
  try {
    ...
    const generationPromise = (async () => {
      ...
      const t0 = Date.now();
      // ⚡ Bolt Optimization: Wrap network request in timeoutPromise with an AbortSignal.
      // This eagerly cancels the underlying fetch if the API hangs, preventing zombie connections
      // from consuming background memory/CPU and ensuring that pending promises in our request
      // coalescing cache (\`pendingTranslations\`) are correctly rejected and cleared.
      const queryVector = await timeoutPromise(
        (signal) => getCloudflareEmbedding(text, cfAccountId, cfApiToken, signal),
        10000,
        "Cloudflare Embedding 响应超时 (10s)"
      );
