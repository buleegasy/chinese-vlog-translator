...
export async function POST(req: Request) {
  ...
  try {
    ...
    const generationPromise = (async () => {
      ...
      const t0 = Date.now();
      // ⚡ Bolt Optimization: Use timeoutPromise with AbortSignal for the Cloudflare API call.
      // This prevents hanging requests from indefinitely blocking the pendingTranslations
      // request coalescing map in Edge environments, avoiding a permanent memory leak/stall.
