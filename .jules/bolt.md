...
## 2026-08-13 - [Request Coalescing Cache Deadlock]
**Learning:** In Edge API routes, using a module-level Map for request coalescing (`pendingTranslations`) is powerful, but extremely dangerous if network requests hang. If a `fetch` call blocks indefinitely without a timeout or explicit cancellation, the pending Promise never resolves or rejects, leaving the request permanently stuck in the Map. This causes a cache deadlock where all future identical requests stall forever waiting on the zombie promise.
**Action:** Always wrap underlying network requests in request coalescing patterns with a strict, `AbortController`-backed timeout. This guarantees the promise will eventually reject, allowing the `finally` block to clear the deadlock and permit subsequent requests to retry.
